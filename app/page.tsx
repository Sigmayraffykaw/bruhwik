...
const faq=[
["What does the download button do?","It downloads a platform-specific BruhWik setup guide immediately. The full desktop app is still in development, so the site never pretends to download software that does not exist yet."],
["Is BruhWik affiliated with Spotify?","No. BruhWik is an independent community customization project and is not affiliated with or endorsed by Spotify AB."],
...
function downloadGuide(os:string){
 const guides:any={
 Windows:`BruhWik for Windows\n\n1. Keep Spotify Desktop installed.\n2. Visit bruhwik.com for verified themes.\n3. The BruhWik desktop installer is currently in development.\n4. Never run unknown scripts from untrusted creators.\n\nThanks for joining BruhWik early.`,
 macOS:`BruhWik for macOS\n\n1. Keep Spotify Desktop installed.\n2. Browse verified themes on bruhwik.com.\n3. The signed macOS client is currently in development.\n4. Only install files from trusted BruhWik releases.`,
 Linux:`BruhWik for Linux\n\n1. Keep Spotify Desktop installed.\n2. Browse verified themes on bruhwik.com.\n3. The Linux package is currently in development.\n4. Official packages will be published with checksums.`};
 const blob=new Blob([guides[os]],{type:"text/plain"}); const url=URL.createObjectURL(blob); const a=document.createElement("a"); a.href=url; a.download=`BruhWik-${os}-Setup.txt`; a.click(); URL.revokeObjectURL(url);
}
...
export default function Home(){
 ...
 return <main className="site-shell">
  ...
  <nav className="nav"><a href="#" className="brand"><span><WandSparkles size={18}/></span>BruhWik<b>.</b></a>
  ...
  <section id="home" className="hero section"><motion.div initial={{opacity:0,y:24}} animate={{opacity:1,y:0}} transition={{duration:.8}} className="hero-copy">
   ...
   <div className="hero-buttons"><a className="primary" href="#download"><Download size={18}/>Download BruhWik</a><a className="secondary" href="#themes"><Sparkles size={18}/>Explore themes</a></div>
  ...
  <footer><a href="#" className="brand"><span><WandSparkles size={18}/></span>BruhWik<b>.</b></a><div>{["Privacy","Terms","GitHub","Discord","Twitter"].map(x=><a key={x} href={x==="GitHub"?"https://github.com/Sigmayraffykaw/bruhwik":"#"}>{x}</a>)}</div><p>Independent community project. Not affiliated with Spotify AB.</p></footer>
 </main>
}
